...
function getDmcColor(r, g, b) {
    const first = dmcColorList[0];
    let distance = Math.sqrt((r - first[2]) * (r - first[2]) + (g - first[3]) * (g - first[3]) + (b - first[4]) * (b - first[4]));
    let color = 0;
    for (let i = 0; i < dmcColorList.length; i++) {
        const dmc = dmcColorList[i];
        let tempDistance = Math.sqrt((r - dmc[2]) * (r - dmc[2]) + (g - dmc[3]) * (g - dmc[3]) + (b - dmc[4]) * (b - dmc[4]));
        if (tempDistance < distance) {
            distance = tempDistance;
            color = i;
        }
    }
    return [dmcColorList[color], color];
}
...
img.onload = function () {
    ...
    convertToDmcImage(img.height, img.width);
}
